...
import { Combo, ComboInput, ComboStatus, CiProductInput, MedProductInput } from '../../types/combo';
import { OrderBumpForm } from '../forms/OrderBumpForm';

const EDIT_COMBO_MUTATION = `
  mutation EditCombo($editComboId: ID!, $combo: ComboInput) {
    editCombo(id: $editComboId, combo: $combo)
  }
`;
...
export function EditComboForm() {
  ...
  const combo = location.state?.combo as Combo;
  const { query } = useApi();
  ...
  const [formData, setFormData] = useState<ComboInput>({
    ...
    status: combo?.status || ComboStatus.ACTIVE,
    price: combo?.price || 0,
    cover: combo?.cover || '',
    singleSale: combo?.singleSale || false,
    currency: combo?.currency || 'BRL',
    language: combo?.language || 'PT_BR',
    bookIds: combo?.bookIds || [],
    courseIds: combo?.courseIds || [],
    cis: combo?.cis || [],
    meds: combo?.meds || [],
    orderBump: combo?.orderBump || [],
  });
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100">
          <div className="p-6">
            <div className="text-center text-red-600">
              Dados do combo não encontrados. Volte para a lista de combos e tente novamente.
            </div>
          </div>
        </div>
      </div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      setError(null);
      await query(EDIT_COMBO_MUTATION, { editComboId: id, combo: formData });
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="border-t pt-6">
              <h3 className="text-lg font-medium mb-4">Gerenciar Produtos</h3>
